...
ERROR_RESPONSES = {
    "MISSING_AUTH": {"status": "error", "error_code": "MISSING_AUTH", "message": "Missing authentication credentials"},
    "INVALID_AUTH": {"status": "error", "error_code": "INVALID_AUTH", "message": "Invalid authentication credentials"},
    "INVALID_SIGNATURE": {"status": "error", "error_code": "INVALID_SIGNATURE", "message": "Invalid webhook signature"},
    "INVALID_CONTENT_TYPE": {"status": "error", "error_code": "INVALID_CONTENT_TYPE", "message": "Content-Type must be application/json"},
    "INVALID_PAYLOAD": {"status": "error", "error_code": "INVALID_PAYLOAD", "message": "Invalid or missing payload data"},
    "VALIDATION_ERROR": {"status": "error", "error_code": "VALIDATION_ERROR", "message": "Payload validation failed"},
    "DATABASE_ERROR": {"status": "error", "error_code": "DATABASE_ERROR", "message": "Database operation failed"},
    "INTERNAL_ERROR": {"status": "error", "error_code": "INTERNAL_ERROR", "message": "Internal server error"},
    # NEW: Bot detection specific errors
    "BOT_DETECTED": {"status": "error", "error_code": "BOT_DETECTED", "message": "Bot activity detected - request rejected"},
    "FAKE_REFERRAL_DETECTED": {"status": "error", "error_code": "FAKE_REFERRAL_DETECTED", "message": "Fake referral detected - request rejected"},
    "BSE_PROCESSING_ERROR": {"status": "error", "error_code": "BSE_PROCESSING_ERROR", "message": "Behavioral scoring engine processing failed"}
}
...
class FakeReferralDetectionService:
    ...
    def detect_fake_referral_patterns(payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"is_fake": len(fake_signals) > 0, "signals": fake_signals, "risk_score": len(fake_signals) * 25}
...
def verify_webhook_signature(payload_body: bytes, signature_header: str) -> bool:
    ...
    try:
        ...
        expected_signature = hmac.new(WEBHOOK_SECRET.encode('utf-8'), payload_body, hashlib.sha256).hexdigest()
...
def verify_webhook_token(auth_header: Optional[str], api_key_header: Optional[str]) -> bool:
    """
    Verify webhook using either:
    ...
    try:
        ...
        if not candidate:
            logger.warning("Missing Authorization or X-Api-Key header")
            return False
        is_valid = hmac.compare_digest(candidate, WEBHOOK_TOKEN)
        if not is_valid:
            logger.warning("API token mismatch (masked preview): %s", (candidate[:4] + "..." + candidate[-4:]) if len(candidate) > 8 else "****")
        return is_valid
    except Exception as e:
        ...
        try:
            ...
            if WEBHOOK_AUTH_METHOD == "signature":
                ...
                if not auth_header and not api_key_header:
                    logger.warning(f"Missing Authorization/X-Api-Key from {request.remote_addr}")
                    return jsonify(ERROR_RESPONSES["MISSING_AUTH"]), 401
                if not verify_webhook_token(auth_header, api_key_header):
                    logger.warning(f"Invalid token/api-key from {request.remote_addr}")
                    return jsonify(ERROR_RESPONSES["INVALID_AUTH"]), 401
                logger.info(f"✅ Valid webhook token/api-key from {request.remote_addr}")
...
def require_bot_detection(f):
    ...
    def decorated_function(*args, **kwargs):
        if not BOT_DETECTION_WEBHOOK_ENABLED:
            return f(*args, **kwargs)
        try:
            bot_signals = BotDetectionService.extract_bot_signals(request.get_json(silent=True) or {}, dict(request.headers))
...
def send_to_bse(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        response = requests.post(BSE_ENDPOINT, json=enhanced_payload, timeout=10, headers={'Content-Type': 'application/json'})
...
def handle_webhook():
    # OPTIONS preflight handled by require_webhook_auth (bypass) and CORS
    if request.method == "OPTIONS":
        return ("", 204)
    ...
    try:
            ...
            try:
                if BSE_ASYNC_PROCESSING:
                    ...
                    if bse_result.get("error"):
                        ...
                        if "risk_flags" in bse_result and isinstance(bse_result["risk_flags"], list):
        ...
        logger.info(f"✅ User {user_id} updated - Score: {behavior_score}, Flags: {len(risk_flags)}, Processing time: {processing_time:.3f}s")
        ...
        log_skipped_payload(request.data if hasattr(request, 'data') else None, f"Unexpected server error: {str(e)}", "INTERNAL_ERROR")
        return jsonify(ERROR_RESPONSES["INTERNAL_ERROR"]), 500
...
def health_check():
    ...
    try:
        ...
        return jsonify({"status": "unhealthy", "timestamp": datetime.utcnow().isoformat() + "Z", "error": str(e)}), 503
...
def webhook_stats():
    ...
    try:
        ...
        return jsonify({
            ...
            "auth_method": WEBHOOK_AUTH_METHOD,
            "bot_detection": {"enabled": BOT_DETECTION_WEBHOOK_ENABLED, **bot_stats},
            "fake_referral_detection": {"enabled": True, **fake_referral_stats},
            "bse_integration": {"enabled": BSE_ENABLED, "async_processing": BSE_ASYNC_PROCESSING},
